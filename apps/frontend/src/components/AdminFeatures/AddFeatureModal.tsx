...
export const AddFeatureModal: React.FC<AddFeatureModalProps> = ({
    ...
    const loadExistingFeatures = async () => {
      if (isOpen) {
        try {
          ...
          setExistingFeatures(response.features.map(f => f.key));
        } catch (error) {
  ...
  const validateForm = (): boolean => {
    ...
    } else if (!AVAILABLE_FEATURES.includes(formData.key)) {
      newErrors.key = `Feature key must be one of the defined constants: ${AVAILABLE_FEATURES.join(', ')}`;
    } else if (existingFeatures.includes(formData.key)) {
      newErrors.key = `Feature with key '${formData.key}' already exists`;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await featuresApi.createFeature(formData);
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center">
          ...
          >
            ...
            <div className="flex items-center justify-between p-6 border-b border-border">
              <h2 className="text-xl font-semibold text-foreground">Add New Feature</h2>
              <ModernButton
                variant="ghost"
                onClick={handleClose}
                className="p-2"
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
              ...
              <div>
                ...
                >
                  ...
                  {AVAILABLE_FEATURES.map(featureKey => (
                    <option
                      key={featureKey}
                      value={featureKey}
                      disabled={existingFeatures.includes(featureKey)}
                    >
                      {featureKey} {existingFeatures.includes(featureKey) ? '(Already exists)' : ''}
                    </option>
                  ))}
                </select>
                {errors.key && (
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  Only feature keys defined in code constants are allowed
                </p>
              </div>
                ...
                <label className="flex items-center gap-3 cursor-pointer">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground">
                      Feature will be globally enabled when created
                    </div>
                  </div>
                </label>
